...
var DEFAULT_INCLUDE_TIRAID_IN_EVENT = true;
var DEFAULT_GPS_BAUDRATE = 4800;
...
function HLCServer(options) {
  ...
  // ----- route: /gps ----- TODO: move to separate file
  self.router.route('/gps').get(function(req, res) {
    if(self.gps) {
      res.json({
        _meta: { message: "ok", statusCode: 200 }, gps: self.gps.state
      });
    }
    else {
      res.status(501).json({
        _meta: { message: "GPS not installed",  statusCode: 501 }
      });
    }
  });
 ...
 * @param {Object} options The options as a JSON object.
 */
HLCServer.prototype.addGPSListener = function(options) {
  var self = this;

  if(!options || !options.hasOwnProperty('path')) {
    console.log('GPS path not specified, aborting listening attempt');
    return;
  }

  // Using GPS requires manually installation of gps and serialport packages
  var GPS = require('gps');
  var SerialPort = require('serialport');

  self.gps = new GPS;
  var gpsPort = new SerialPort(options.path, { 
    baudrate: options.baudrate || DEFAULT_GPS_BAUDRATE,
...
function parseOptions(instance, options) {
  ...
  if(!options.barnacles.hasOwnProperty('includeTiraidInEvent')) {
    options.barnacles.includeTiraidInEvent = DEFAULT_INCLUDE_TIRAID_IN_EVENT;
  }
