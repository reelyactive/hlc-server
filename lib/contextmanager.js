...
var MAX_REELCEIVERS = 3;
var MAX_STALE_MILLISECONDS = 10000;
var TEST_PLACE = "test";
...
function ContextManager(options) {
  var self = this;
  this.maxDecoders = options.maxDecoders || MAX_REELCEIVERS;
  ...
  setInterval(periodicMaintenance, this.maxStaleMilliseconds);
};
...
ContextManager.prototype.findNearby = function(params, callback) {
  ...
  this.db.find({ $or: [{ _id: { $in: params.ids } },
    ...
    for(var cId = 0; cId < matching.length; cId++) {
      ...
      var latestTransmission = transmissions[transmissions.length - 1];
      var latestIdentifier = latestTransmission.identifier;
      var identifierValue = latestIdentifier.value;
      var isMatchingTransmitter = (params.ids.indexOf(identifierValue) != -1);
      if(isMatchingTransmitter) {
        nearby[identifierValue] = latestTransmission;
      }
      else {
